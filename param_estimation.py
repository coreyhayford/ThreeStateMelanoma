...
bestrates=answ[0]
bestscore=answ[1]
k_div_A, k_div_B, k_div_C, k_death_A, k_death_B, k_death_C, k_AB, k_BA, k_CB, k_BC=answ[0]
newrates=(k_div_A, k_div_B, k_div_C, k_death_A, k_death_B, k_death_C, k_AB, k_BA, k_CB, k_BC)
print newrates

### Generate Solution to System ###
F0,F1,F2,T=eq(newrates,y_0,start_time,end_time,intervals)
Zm=np.log2((F0+F1+F2)/(F0[0]+F1[0]+F2[0]))[mindex]
Tm=T[mindex]
...
title='Model_Data + Fit Score: '+str(bestscore)
plt.title(title)
