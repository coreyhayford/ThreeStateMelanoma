...
mixing_all = pd.read_csv('/Users/Corey/Documents/QuarantaLab/Mixing/mixing_all_psg13', sep = "\t")
...
mixing.append(mixing_all.loc[mixing_all['CellLine'] == "Subclone01+Subclone07+Subclone10"])
# print mixing
# quit()
...
bounds = [(None, 0), (-0.01, 0.01), (0, None), (0, None), (0, None), (0, None), (0, None)]
print len(bounds)
...
# Need to get variance around 0 (+ only) and get model to search that space
# for i in range(len(bounds), len(x0)):
#     bounds.append((x0[i]-300, x0[i]+300))

# for i in range(len(bounds)+10, len(bounds)+18):
#     bounds.append((x0[i]-150, x0[i]+150))
#  
# for i in range(len(bounds)+19, len(bounds)+21):
#     bounds.append((x0[i]-100, x0[i]+100)) 
print x0
print bounds
print len(bounds)
print len(x0)
...
red_patch = mpatches.Patch(color = "red", label = "Initial Model")
blue_patch = mpatches.Patch(color = "blue", label = "Experimental Average")
green_patch = mpatches.Patch(color = "green", label = "Optimized Model")
grey_star = mlines.Line2D([], [], color = 'grey', marker="*", markersize=12, 
...
for i, ic in enumerate(ICs()):
    ...
    plt.legend(handles = [grey_star, blue_patch, red_patch, green_patch], loc = 'upper left')
plt.show()