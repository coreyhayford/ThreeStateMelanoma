...
# for i,p in enumerate(["Hi", "how", "are", "you"]):
#     good = ["1", "2", "3", "4"]
#     print p
#     chart = np.column_stack([p, good])
#     print chart

    #np.savetxt('/Users/Corey/Documents/QuarantaLab/PE_010710', chart, delimiter = '\t', fmt = '%s')
...
#data_parental_250 = pd.read_csv('/Users/Corey/Documents/QuarantaLab/mixing_parental_250', sep = "\t")
#print data_parental_250

mixing_all = pd.read_csv('/Users/Corey/Documents/QuarantaLab/mixing_all_250', sep = "\t")
#print mixing_all
...
print(range(0, len(mixing_10), 19))
sublist = [mixing_10[x:x+19] for x in range(0, len(mixing_10), 19)]

rep1 = sublist[0] # first 10 data points
rep2 = sublist[1]
rep3 = sublist[2]
...
bounds = [(None, None), (0, 0), (None, None), (0, None), (0, None), (0, None), (0, None)] # hard coded - should change
#print len(bounds)
...
for i,p in enumerate(model.parameters[3:]):
    print p,result.x[i]
    #chart = np.column_stack([p, result.x[i]])
    #np.savetxt('/Users/Corey/Documents/QuarantaLab/PE_010710', chart, delimiter = '\t', fmt = '%s')
    
    # Send the parameters to a csv file - preferably in a loop so can put on same chart
...
plt.title('Parameter Estimation (Mixing 10)')
plt.show()
